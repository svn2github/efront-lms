				...
				if ($this -> course['issued_certificate']) {
					...
					if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
						if ($this -> course['certificate_expiration'] != "" && $this -> course['certificate_expiration'] != 0) {
							...
							$dateExpire = date($dateFormat, $expirationTimestamp);
							$tooltipCourseString = "<strong>". _CERTIFICATEISSUEDON.":</strong>&nbsp;".date($dateFormat,$dateTable['date'])."<br /><strong>".($expirationTimestamp > time() ? _CERTIFICATEEXPIRESON : _CERTIFICATEEXPIREDON).":</strong>&nbsp;".$dateExpire."<br /><strong>"._RESETCOURSEAFTEREXPIRATION.":</strong>&nbsp;".($this -> course['reset'] == 1 ? _YES : _NO);

							if ($expirationTimestamp > time()) {
								$courseString .= '<a onmouseover="document.getElementById(\'certificate_info_'.$this -> course['id'].'\').style.display = \'inline\';" href = "student.php?ctg=lessons&course='.$this -> course['id'].'&export=rtf&user='.$this -> userStatus['login'].'" target="_blank">
								...
								$courseString .= '<a onmouseover="document.getElementById(\'certificate_info_'.$this -> course['id'].'\').style.display = \'inline\';" href = "javascript:void(0)">
														<img src = "images/16x16/certificate.png" title = "'._COURSECERTIFICATE.'" alt = "'._COURSECERTIFICATE.'" border = "0" style = "vertical-align:middle"></a>';
							}
							$courseString .= "<a onMouseOut=\"document.getElementById('certificate_info_".$this -> course['id']."').style.display = 'none';\" id='certificate_info_".$this -> course['id']."' style='display:none' class = 'info' href = 'javascript:void(0)'>--><img class = 'tooltip' border = '0' src='images/others/tooltip_arrow.gif'/><span class = 'tooltipSpan'>".$tooltipCourseString."</span></a>";
						} else {
							$courseString .= '<a href = "student.php?ctg=lessons&course='.$this -> course['id'].'&export=rtf&user='.$this -> userStatus['login'].'" target="_blank">
													<img src = "images/16x16/certificate.png" title = "'._COURSECERTIFICATE.'" alt = "'._COURSECERTIFICATE.'" border = "0" style = "vertical-align:middle"></a>';
						}
