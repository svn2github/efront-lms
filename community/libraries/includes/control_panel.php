  ...
  $smarty -> assign("T_CONTROL_PANEL_GROUPS", $controlPanelGroups);
        ...
        if ($_admin_) {
            ...
            if (!isset($currentUser -> coreAccess['logout_user']) || $currentUser -> coreAccess['logout_user'] == 'view') {
                $controlPanelOptions[] = array('text' => _ONLINEUSERS, 'image' => "32x32/logout.png", 'href' => "administrator.php?ctg=logout_user&popup=1", 'onClick' => "eF_js_showDivPopup('"._LOGOUTUSER."', 0)", 'target' => 'POPUP_FRAME');
            }
            if (!isset($currentUser -> coreAccess['users']) || $currentUser -> coreAccess['users'] != 'hidden') {
