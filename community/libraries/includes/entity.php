...
if (isset($_GET['delete']) && in_array($_GET['delete'], $legalValues) && $_change_) {
    ...
    try {
        ...
        if (!isset($form) || !($form instanceof HTML_QuickForm)) {
            $form = new HTML_QuickForm("create_form", "post", basename($_SERVER['PHP_SELF'])."?ctg=$entityName".(isset($_GET['add']) ? '&add=1' : '&edit='.$_GET['edit']), "", null, true);
        }
	    ...
	    try {
	        ...
	        if ($form -> isSubmitted() && $form -> validate()) {
	            ...
	            if (isset($values) && $values) {
	                $entity -> handleForm($form, array_merge($form -> exportValues(), $values));
	            } else {
	                $entity -> handleForm($form);
	            }
