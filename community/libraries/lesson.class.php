...
//                        'publish'         => $this -> lesson['publish'],
  ...
  if (!($GLOBALS['configuration']['social_modules_activated'] & SOCIAL_FUNC_LESSON_TIMELINES)) {
     ...
     if ($topic_ID) {
      ...
      $related_events = eF_getTableData("events", "*", "type = '".EfrontEvent::NEW_POST_FOR_LESSON_TIMELINE_TOPIC. "' AND entity_ID = '".$topic_ID."' AND lessons_ID = '". $this->lesson['id']."' AND users_LOGIN IN ('".implode("','", $users_logins)."')", "timestamp desc");
        } else {
      // only current lesson users
      $users = $this -> getUsers();
      $users_logins = array_keys($users);
      if ($limit) {
       ...
       $related_events = eF_getTableData("events", "*", "lessons_ID = '". $this->lesson['id']."' AND users_LOGIN IN ('".implode("','", $users_logins)."')", "timestamp desc");
      }
        }
   ...
   $user = $users[$event['users_LOGIN']];
         try {
             ...
             $related_events[$key]['avatar'] = $user['avatar'];
             list($related_events[$key]['avatar_width'], $related_events[$key]['avatar_height']) = eF_getNormalizedDims($file['path'],$avatarSize, $avatarSize);
         } catch (EfrontfileException $e) {
             $related_events[$key]['avatar'] = G_SYSTEMAVATARSPATH."unknown_small.png";
             $related_events[$key]['avatar_width'] = $avatarSize;
             $related_events[$key]['avatar_height'] = $avatarSize;
         }
     }
        if ($returnObjects) {
            $eventObjects = array();
