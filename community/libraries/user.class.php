...
abstract class EfrontUser
{
	...
	public static function createUser($userProperties) {
		...
		if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
			if (G_VERSIONTYPE != 'standard') { #cpp#ifndef STANDARD
				if (isset($GLOBALS['configuration']['version_users']) && sizeof($activatedUsers) > $GLOBALS['configuration']['version_users'] && $GLOBALS['configuration']['version_users'] > 0) {
		...
		if (eF_insertTableData("users", $userProperties)) {
			...
			if (mkdir($user_dir, 0755) || is_dir($user_dir)) {                            //Now, the directory either gets created, or already exists (in case errors happened above). In both cases, we continue
				...
				if (G_VERSIONTYPE == 'educational' || G_VERSIONTYPE == 'enterprise') { #cpp#ifndef COMMUNITY
					EfrontGroup::addToDefaultGroup($newUser);
				} #cpp#endif
	...
	public function activate() {
		if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
			if (G_VERSIONTYPE != 'standard') { #cpp#ifndef STANDARD
				$users = eF_getTableDataFlat("users", "*","active=1");
				if (isset($GLOBALS['configuration']['version_users']) && sizeof($users['login']) > $GLOBALS['configuration']['version_users'] && $GLOBALS['configuration']['version_users'] > 0) {
		...
		foreach ($groupIds as $key => $groupId) {
			if (eF_checkParameter($groupId, 'id')) {
				...
				if (!isset($this -> groups[$groupId])) {
					...
					if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
						EfrontEvent::triggerEvent(array("type" => EfrontEvent::NEW_ASSIGNMENT_TO_GROUP, "users_LOGIN" => $this -> user['login'], "users_name" => $this -> user['name'], "users_surname" => $this -> user['surname'] , "entity_ID" => $groupId, "entity_name" => $group_names[$groupId]));
					} #cpp#endif
