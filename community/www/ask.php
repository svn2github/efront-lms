			...
			if ($_SESSION['s_type'] == "administrator") {
				...
				if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
					if (isset($user ->aspects['hcd']) && $user ->aspects['hcd']->isSupervisor()) {
						...
						//  [supervises_branches] => 2,3,4,5,6,7,11,1

						include "../libraries/module_hcd_tools.php";

						//$supervisor_at_branches = $_SESSION['supervises_branches'];
						//pr($_SESSION);    // OR EXISTS (select module_hcd_employees.users_login from module_hcd_employees LEFT OUTER JOIN module_hcd_employee_works_at_branch ON module_hcd_employee_works_at_branch.users_login = module_hcd_employees.users_login where users.login=module_hcd_employees.users_login AND module_hcd_employee_works_at_branch.branch_ID IS NULL)
						//$supervised_employees = eF_getTableData("users LEFT OUTER JOIN module_hcd_employee_has_job_description ON users.login = module_hcd_employee_has_job_description.users_LOGIN LEFT OUTER JOIN module_hcd_employee_works_at_branch ON users.login = module_hcd_employee_works_at_branch.users_LOGIN","users.login","(users.user_type <> 'administrator' AND ((module_hcd_employee_works_at_branch.branch_ID IN (" . $_SESSION['supervises_branches'] ." ) AND module_hcd_employee_works_at_branch.assigned='1'))) AND active = 1 AND (login like '$preffix%' OR name like '$preffix%' OR surname like '$preffix%' OR user_type like '$preffix%') GROUP BY login", "login");
						//AND (login like '$preffix%' OR name like '$preffix%' OR surname like '$preffix%' OR user_type like '$preffix%')
						...
						//$supervised_employees = $supervised_employees['login'];
