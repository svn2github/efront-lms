...
if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
    ...
    if (($GLOBALS['configuration']['social_modules_activated'] & FB_FUNC_LOGGING)) {
        ...
        if ($GLOBALS['configuration']['facebook_api_key'] && $GLOBALS['configuration']['facebook_secret']) {
            $facebook_inteface = 1;
            if (!isset($_GET['logout']) && isset($_COOKIE[$GLOBALS['configuration']['facebook_api_key'] . '_session_key'])) {
                	
                // The following two used for login button appearance
                $smarty -> assign("T_NO_FACEBOOK_LOGIN", 1);
                $facebook_inteface = 0;
                	
                	
                new EfrontFacebook();
                $eF_user = eF_getTableData("facebook_connect JOIN users ON users.login = facebook_connect.users_LOGIN", "users_LOGIN, password", "fb_uid='".$_SESSION['facebook_user']."'");
                ...
                try {
                    ...
                    if (!isset($_GET['ctg'])) {
                        ...
                        $fb_form_existing = new HTML_QuickForm("facebook_login_form", "post", basename($_SERVER['PHP_SELF']), "", "class = 'indexForm'", true);
...
if (isset($_GET['ctg']) && ($_GET['ctg'] == "signup") && $configuration['signup']) {
	...
	if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
	    if (G_VERSIONTYPE != 'standard') { #cpp#ifndef STANDARD
	        if (sizeof($users['login']) > $GLOBALS['configuration']['version_users'] && $GLOBALS['configuration']['version_users'] > 0) {
	    	...
	    	if ($field['type'] != 'branchinfo') {
	    		...
	    		if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
			        ...
			        if (!empty($branches)) {
			        	...
			        	$form -> addElement('select', $field['name'].'_branches' , $options['name'], $branches ,'class = "inputText" id="'.$field['name'].'_branches" name="'.$field['name'].'_branches"  onchange="javascript:change_branch(\''.$field['name'].'_branches\', \''.$field['name'].'_jobs\');change_supervisors(\''.$field['name'].'_branches\',\''.$field['name'].'_supervisors\');"');
			        	...
			        	if ($options['job']['description'] != "") {
				        	...
				        	if ($options['job']['description'] != "") {
					            ...
					            if (MSIE_BROWSER) {
					                $form -> addElement('select', $field['name'].'_jobs' , $job_description_field, array("" => _PLEASESELECT . " " .$options['name']), 'class="inputText" id= "'. $field['name'].'_jobs" onFocus = "this.selIndex = this.selectedIndex;" onChange="restoreSelection(this);" id= "jobs_main"  ' . $hidden_job);
					            } else {
					                $form -> addElement('select', $field['name'].'_jobs' , $job_description_field, array("" => _PLEASESELECT . " " .$options['name']), 'class="inputText" id= "'. $field['name'].'_jobs"  ' . $hidden_job);
					            }
				        	}
			        	}
			            ...
			            if ($configuration['supervisor_mail_activation'] == 1 && $options['supervisor']['description'] != "") {
			            	...
			            	if ($options['supervisor']['hidden'] == 1) {
				        		$form -> addElement('select', $field['name'].'_supervisors' , "", array("" => _PLEASESELECT . " " .$options['name'] ), 'class="inputText" id= "'.$field['name'].'_supervisors" style="display:none"');	
				        	} else {
				        		$form -> addElement('select', $field['name'].'_supervisors' , $options['supervisor']['description'], array("" => _PLEASESELECT . " " .$options['name'] ), 'class="inputText" id= "'.$field['name'].'_supervisors" ');
				        	}			            	
