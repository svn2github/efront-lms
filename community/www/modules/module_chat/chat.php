...
function chatHeartbeat() {
 $sql = "select * from module_chat where (module_chat.to_user = '".mysql_real_escape_string($_SESSION['chatter'])."' AND recd = 0) order by id ASC";
...
EOD;
 ...
 $sql = "insert into module_chat (module_chat.from_user,module_chat.to_user,message,sent) values ('".mysql_real_escape_string($from)."', '".mysql_real_escape_string($to)."','".mysql_real_escape_string($message)."',NOW())";
 $query = mysql_query($sql);
 echo "1";
 exit(0);
}
