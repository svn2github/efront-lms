...
class FUZE_UserDAO extends FUZE_AbstractDAO {
 ...
 public function _init() {
  ...
  if (is_array($res) && count($res)) {
   ...
   $this->_to->set('creator', $res[0]['creator']);
   ...
   if (isset($args['subject']) && !empty($args['subject']) &&
     ...
     if ( !$args['launch'] ) {
      ...
      if (is_array($response)) {
       if (isset($response['launchmeetingurl']) && !empty($response['launchmeetingurl']) &&
       ...
       // Either a failure or a redirect to the upgrade page
 ...
 public function getUpgradeUrl($args) {
  $function_response = array('success' => false);
  if (is_array($args) && count($args)) {
   if (isset($args['fuze_email']) && !empty($args['fuze_email']) &&
       isset($args['fuze_passwd']) && !empty($args['fuze_passwd'])
    ) {
     $options = array();
     $options [Request_Adapter_Abstract::REQUEST_ADAPTER_REQUEST_TYPE] = Request_Adapter_Abstract::REQUEST_ADAPTER_REQUEST_TYPE_USER_LOGIN;
     $options [Request_Adapter_Abstract::REQUEST_ADAPTER_REQUEST_PARAMS] = array();
     ...
     if (is_array($response) && isset($response ['upgrade_url']) && !empty($response ['upgrade_url'])) {
      $function_response ['success'] = true;
      $function_response ['url'] = $response ['upgrade_url'];
     }
     else {
