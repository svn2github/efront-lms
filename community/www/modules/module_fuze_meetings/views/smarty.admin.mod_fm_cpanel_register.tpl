  ...
  new Ajax.Request(url, {
   ...
   onSuccess: function(response) {
    ...
    if (response.success && response.professor_ids.length && response.professor_names.length) {
     ...
     html += '	<tr>';
     html += '		<td style="padding:1px;">';
     html += '			<input type="button" class="flatButton" id="mod_fm_admin_user_remove_btn" value={/literal}"{$smarty.const._FUZE_ADMIN_USER_SUSPEND_BTN}"{literal} onclick="javascript:_mod_fm_user_suspend();" disabled/>';
     html += '		</td>';
     html += '	</tr>';
     html += '	<tr>';
     html += '		<td style="padding:1px;">';
