...
<!--ajax:sharedFilesTable-->
 <table class = "sortedTable" sortBy = "0" style = "width:100%" size = "{$T_TABLE_SIZE}" id = "sharedFilesTable" useAjax = "1" rowsPerPage = "20" other = "{$T_CURRENT_DIRECTORY}" url = "{$smarty.server.PHP_SELF}?ctg=control_panel&" nomass = "1" currentDir = "{$T_DIR_PATH}">
  <tr>
   ...
   <td class = "topTitle centerAlign noSort" >{$smarty.const._DOWNLOADFILE}</td>
  </tr>
  {if $T_PARENT_DIR}
  ...
  <tr class = "defaultRowHeight {cycle values = "oddRowColor, evenRowColor"}">
   ...
   <td class = "centerAlign">{if $file.type == 'file'}<a href = "view_file.php?file={$file.path}&action=download"><img src = "images/16x16/import.png" alt = "{$smarty.const._DOWNLOADFILE}" title = "{$smarty.const._DOWNLOADFILE}"/></a>{/if}</td>
  </tr>
  {foreachelse}
