...
function smarty_outputfilter_eF_template_applyImageMap($compiled, &$smarty) {
    ...
    //Now, replace image src tag with transparent.png and add the image map classes, 'spriteXX spriteXX-imagename'
    $matches = array('/(<img .*)(?<!\/)\Wimages\/16x16(\/.*)?\/((.*)\.\w{3}\W)/U',
         '/(<img .*)(?<!\/)\Wimages\/32x32(\/.*)?\/((.*)\.\w{3}\W)/U');
    $replacements = array("$1'images/others/transparent.png' class = 'sprite16 sprite16-$4'",
           "$1'images/others/transparent.png' class = 'sprite32 sprite32-$4'");
