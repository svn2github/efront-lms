    ...
    {elseif $T_COURSE_INFO}
        {capture name = 't_lesson_info_code'}
        	...
        	{if $T_COURSE_LESSONS}
        		...
        		<a href = "{$smarty.server.PHP_SELF}?{if $smarty.get.ctg}ctg={$smarty.get.ctg}&{/if}lessons_ID={$item.id}&course={$smarty.get.courses_ID}" class = "editLink">{$item.name}{if !$smarty.foreach.course_lessons_list.last},{/if}</a>
        		{/foreach}
        		</div>
        	{/if}
