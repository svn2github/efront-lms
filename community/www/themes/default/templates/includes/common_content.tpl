 ...
 {else}
  ...
  {capture name = "t_content_tools"}
        ...
        {capture name = "moduleShowUnit"}
            ...
            <script>var nextUnit = '{$T_NEXT_UNIT.id}';var previousUnit = '{$T_PREVIOUS_UNIT.id}';{if $T_UNIT.options.no_before_unload}var noBeforeUnload = true;{else}var noBeforeUnload = false;{/if}</script>{*The first 2 are needed for nextUnit()/previousUnit() js functions and the last for scorm function*}
