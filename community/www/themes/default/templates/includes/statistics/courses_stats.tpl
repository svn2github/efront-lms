...
{*
                 	<td class = "filter">
                        <select style = "vertical-align:middle" name = "group_branch" onchange = "if (this.options[this.selectedIndex].value != '') document.location='{$smarty.server.PHP_SELF}?ctg=statistics&option=course&{if (isset($smarty.get.tab))}&tab={$smarty.get.tab}{else}&tab=users{/if}&sel_course={$T_COURSE_ID}{if (isset($smarty.get.group_filter))}&group_filter={$smarty.get.group_filter}{/if}{if (isset($smarty.get.user_filter))}&user_filter={$smarty.get.user_filter}{/if}&branch_filter='+this.options[this.selectedIndex].value;">                        
                            {foreach name = "branch_options" from = $T_BRANCHES item = 'branch' key='id'}                       
                                <option value = "{$id}" {if $smarty.get.branch_filter == $id}selected{/if}>{$branch}</option>
                            {/foreach}
                        </select>
                    </td>
*}                    
