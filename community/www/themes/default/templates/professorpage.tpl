...
{if $T_SIDEBAR_MODE == 0}
	<frameset framespacing = "0" frameborder = "0" border="no" id = "framesetId" cols = "{$T_SIDEFRAME_WIDTH}, *"> 	
		...
		{if $smarty.get.view && $smarty.get.view == 'organization'}
		  	{if isset($T_SIDEBAR_URL)}
		  		<frame name = "sideframe" src ="{$T_SIDEBAR_URL}" scrolling="no"/>	
		  	{else}	
		        <frame name = "sideframe" src ="new_sidebar.php?hcd=1" scrolling="no"/>
		    {/if}    
			...
			{else}		
				{if $smarty.get.message && $smarty.get.message_type}
					<frame name = "mainframe" src ="professor.php?ctg=module_hcd"/>
				{else}
					<frame name = "mainframe" src ="professor.php?ctg=module_hcd&message={$smarty.get.message}&message_type={$smarty.get.message_type}"/>
				{/if}
			{/if}		
		{else}
			{if isset($T_SIDEBAR_URL)}
			...
			{else}      
				{if $smarty.get.message && $smarty.get.message_type}
					<frame name = "mainframe" src ="professor.php?message={$smarty.get.message}&message_type={$smarty.get.message_type}"/>
				{else}
					<frame name = "mainframe" src ="professor.php"/>
				{/if}	
			{/if}	
		{/if}
