...
class EfrontUnit extends ArrayObject
{
    ...
    public function persist() {
		...
		EfrontSearch :: insertText($fields['data'], $this['id'], "content", "data");
		EfrontSearch :: removeText('content', $fields['id'], 'title');										//Refresh the search keywords
		EfrontSearch :: insertText($fields['name'], $this['id'], "content", "title");
   ...
   /**
    ...
    public function getFiles($returnObjects = false) {        
        ...
        $filesId = $matchesId[1];
        preg_match_all("#content/lessons/(.*)\"#U", $data, $matchesPath);
        $filesPath = $matchesPath[1];
...
{   
    ...
    public function markSeenNodes($user) {
        ...
        $result        = eF_getTableData("content c, completed_tests ct, tests t", "t.content_ID, ct.status, ct.timestamp", "c.id = t.content_ID and c.lessons_ID = ".$this -> lessonId." and ct.tests_ID = t.id and ct.users_LOGIN='$login'");       
        foreach ($result as $value) {
    ...
    public function copyTest($testId, $targetUnit = false){
        ...
        foreach ($questions as $oldQuestion){
            ...
            unset($question_array['type_icon']);
            unset($question_array['explanation']);
            unset($question_array['plain_text']);
            $newQuestion = Question :: createQuestion($question_array);
            $qid = $newQuestion -> question['id'];   
            $questions_in[$qid] = $oldTest -> getAbsoluteQuestionWeight($oldQuestion -> question['id']);   
        }
     ...
     * @param EfrontUnit $sourceUnit The unit object to be copied
     * @param mixed $targetUnit The id of the parent unit (or the parent EfrontUnit)in which the new unit will be copied, or false (the unit will be appended at the end)
     * @param bool $ucopyFiles Whether to copy the files in the unit
     * @return EfrontUnit The newly created unit object
     * @since 3.5.0
     * @access public
     */
    public function copySimpleUnit($sourceUnit, $targetUnit = false, $copyFiles = true){
    	if (!($sourceUnit instanceof EfrontUnit)) {
    		$sourceUnit = new EfrontUnit($sourceUnit);
    	}
        ...
        if ($copyFiles){
            ...
            foreach ($files as $file){                
                try {
                    ...
                    $data = preg_replace("#content/lessons/".$sourceUnit['lessons_ID']."/(.*)#", "content/lessons/".$this -> lessonId.'/${1}', $data);
    ...
    public function toHTML($iterator = false, $treeId = false, $options = array()) {
        ...
        while ($iterator -> valid()) {
            ...
            if (isset($options['edit']) && $options['edit']) {
                $editLink = '<a href = "'.basename($_SERVER['PHP_SELF']).'?ctg='.$contentType.'&edit_unit='.$current['id'].'"><img style = "vertical-align:middle" src = "images/16x16/edit.png" title = "'._EDIT.'" alt = "'._EDIT.'" border = "0" /></a>';
            }
            ...
            $toolsString  = '<span>'.$activateLink.$editLink.$deleteLink.$options['custom'][$current['id']].'</span>';            
            $treeString  .= '
                <li style = "white-space:nowrap;" class = "'.implode(" ", $liClass).'" id = "node'.$current['id'].'" noDrag = "'.$nodrag.'" noRename = "true" noDelete = "true">
                    <a '.($options['onclick'] ? 'onclick = "'.$options['onclick'].'"' : null).' class = "'.(!$current['active'] ? 'treeinactive' : '').' treeLink '.implode(" ", $linkClass).'" href = "'.$targetLink.'" title = "'.$fullName.' '.implode(" ", $tooltip).'">'.$unitName."</a>&nbsp;".$toolsString;
...
class EfrontVisitableFilterIterator extends FilterIterator
{
    ...
    function accept() {
        $current = $this -> current();
        return $current instanceof ArrayObject && ($current['active'] == 1 && $current['publish'] == 1 && ($current['data'] != '' || $current['ctg_type'] == 'tests' || $current['ctg_type'] == 'scorm_test'));
    }
}
