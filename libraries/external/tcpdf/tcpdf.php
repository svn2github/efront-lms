...
if(!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.0.011 (http://www.tcpdf.org)');
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		*/
		protected function utf8Bidi($ta, $forcertl=false) {
			global $unicode, $unicode_mirror, $unicode_arlet, $laa_array;
			...
			// Arabic Shaping (fixed by Saleh AlMatrafe)
			// Cursively connected scripts, such as Arabic or Syriac, require the selection of positional character shapes that depend on adjacent characters. Shaping is logically applied after the Bidirectional Algorithm is used and is limited to characters within the same directional run. 
			if ($arabic) {
				$endedletter = array(1569,1570,1571,1572,1573,1575,1577,1583,1584,1585,1586,1608);
				...
				for ($i=0; $i < $numchars; $i++) {
					...
					if ($unicode[$thischar['char']] == 'AL') {
						// if laa letter
						if(($chardata[($prevALchar)]['char']==1604) AND (in_array($thischar['char'], $alfletter))) {
							$arabicarr = $laa_array;
							$prevchar  = $chardata[($prevALchar-1)];
							$laaletter = $prevALchar;
						} else {
							$arabicarr = $unicode_arlet;
							$laaletter = 0;
						}
						// if previous char is vowels
						if($unicode[$prevchar['char']] == 'NSM'){
							$prevchar = $chardata[($i-2)];
						}
						// if next char is vowels
						if($unicode[$nextchar['char']] == 'NSM'){
							$nextchar = $chardata[($i+2)];
						}
						...
						// if laa letter
						if($laaletter) {
							$chardata2[($laaletter)]['char'] = "0";
							$chardata2[($laaletter)]['type'] = 'B';
						}
					}
				}
				$chardata = $chardata2;
