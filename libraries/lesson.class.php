...
class EfrontLesson
{
    ...
    function __construct($lesson) {
        ...
        if ($this -> lesson['price']) {                                        //Create the string representing the lesson price
            $recurringOptions = array('day' => _DAY, 'week' => _WEEK, 'month' => _MONTH, 'year' => _YEAR);            
            $this -> lesson['price_string'] = $this -> lesson['price'].$GLOBALS['CURRENCYSYMBOLS'][$GLOBALS['configuration']['currency']].($this -> options['recurring'] ? ' / '.$recurringOptions[$this -> options['recurring']] : null);
        }
    }
    ...
    public static function createLesson($fields) {
        ...
        while (is_dir(G_LESSONSPATH.$lessonId) && $q++ < 10000) {                              //$q is put here to prevent infinite loops
        ...
        if (!$fields['course_only']) {
            // Insert the corresponding lesson skill to the skill and lesson_offers_skill tables
            $lessonSkillId = eF_insertTableData("module_hcd_skills", array("description" => _KNOWLEDGEOFLESSON . " ". $fields['name'], "categories_ID" => -1));
            eF_insertTableData("module_hcd_lesson_offers_skill", array("lesson_ID" => $lessonId, "skill_ID" => $lessonSkillId));
        }
    ...
    public function delete() {
        ...
        eF_deleteTableData("glossary_words", "lessons_ID=$id");
        ...
        if ($list) {
            ...
            if ($tests_list) {
                ...
                $result          = eF_getTableDataFlat("done_tests", "id", "tests_ID IN ($tests_list)");
                $done_tests_list = implode(",", $result['id']);
                //$list_as_text    = "'".str_replace(", ", "', '", $list)."'";
        ...
        // Delete the skills correlated with this lesson
        ...
        for ($i = 0; $i < sizeof($courses); $i++) {
            ...
            if (($key = array_search($this -> lesson['id'], $course_lessons)) !== false) {
    ...
    public function getUsers($basicType = false, $refresh = false) {
        if ($this -> users === false || $refresh) {                //Make a database query only if the variable is not initialized, or it is explicitly asked
            $this -> users = array();
            $result = eF_getTableData("users, users_to_lessons", "users.*, users_to_lessons.user_type as role", "users_LOGIN = login and lessons_ID=".$this -> lesson['id']);
    ...
    public function getCourses($returnObjects = false) {
        $result  = eF_getTableData("courses, lessons_to_courses", "courses.*", "lessons_ID = ".$this -> lesson['id']);
        $courses = array();
        foreach ($result as $value) {
        ...
        foreach ($login as $key => $value) {
            if (!in_array($value, array_keys($lessonUsers))) {
                if (eF_checkParameter($value, 'login')) {
                    ...
                    next($role);
                    if (!eF_insertTableData("users_to_lessons", $fields)) {
                        $errors[] = _CANNOTADDUSERTOLESSON.' ('.EfrontLessonException :: DATABASE_ERROR.')';
                    } else {
	...
	*/
    ...
    public function getInformation($user = false, $information = false) {
        ...
        $theoryIds = array();
        foreach (new EfrontVisitableFilterIterator(new EfrontNodeFilterIterator(new RecursiveIteratorIterator(new RecursiveArrayIterator($lessonContent -> tree), RecursiveIteratorIterator :: SELF_FIRST), array('ctg_type' => 'theory'))) as $key => $value) {
...
//        }
        ...
        if ($this -> lesson['info']) {
            unserialize($this -> lesson['info']) !== false ? $info = array_merge($info, unserialize($this -> lesson['info'])) : $info = array_merge($info, $this -> lesson['info']);
        }
    ...
    public function getStatisticInformation($user = false) {
        ...
        if ($this -> lesson['info']) {
            if (unserialize($this -> lesson['info']) !== false) {
                $info = array_merge($info, unserialize($this -> lesson['info']));
    ...
    public function initialize($deleteEntities) {
        ...
        if ($deleteEntities == 'all') {
            $deleteEntities = array('content', 'surveys', 'tracking', 'files',
                                    ...
                                    'glossary', 'users', 'comments', 'rules', 'questions');
        }
        ...
        in_array('glossary',   $deleteEntities) ? eF_deleteTableData("glossary_words",    "lessons_ID=".$this -> lesson['id']) : null;
        ...
        // MODULES - Initialize module table data
        // Get all modules (NOT only the ones that have to do with the user type)
        $modules = eF_loadAllModules();
        foreach ($modules as $module) {
   ...
   /*
    ...
    public function toXML($flgContent){
        ...
        $glossary = ef_getTableData("glossary_words","*","lessons_ID=".$this->lessonId);
    ...
    public function toIMS($path){
        ...
        //write out the glossary
        $glossary = ef_getTableData("glossary_words","*","lessons_ID=".$this->lesson['id']);
        if (sizeof($glossary) > 0){
            $str .= '<glossary>';
            for ($i = 0; $i < sizeof($glossary); $i++){
                $str .= '<word>';
    ...
    //      }
        ...
        for ($i = 0; $i < sizeof($xml->glossary->word); $i++){
            ...
            $cid = ef_insertTableData("glossary_words", $glossary);
        }
     ...
     */
    public function import($file, $deleteEntities = false) {
        ...
        if (isset($replaceString)) {
            for ($i = 0; $i < sizeof($content_data); $i++) {
                ...
                eF_updateTableData("content", array('data' => addslashes($replaced)), "id=".$content_data[$i]['id']);
     ...
     * @access public
     */
    public function export($exportEntities) {

        if (!$exportEntities == 'all') {
            $exportEntities = array('export_surveys', 'export_announcements', 'export_glossary',
                                    'export_calendar', 'export_periods', 'export_comments', 'export_rules');
        }
        ...
        if (isset($exportEntities['export_glossary'])) {
            $glossary_words = eF_getTableData("glossary_words", "*", "lessons_ID = ".$this -> lesson['id']);
    ...
    public static function getLessons() {
        $result = eF_getTableData("lessons l, directions d", "l.*, d.name as direction_name", "l.directions_ID=d.id");
        foreach ($result as $value) {
     ...
     * </code>
     *
     * @parm array $options An array of lesson options
     * @since 3.5.0
     * @access public
     */
...
//                        'publish'         => $this -> lesson['publish'],
                        'from_timestamp'  => $this -> lesson['from_timestamp'],
                        'to_timestamp'    => $this -> lesson['to_timestamp'],
    ...
    public function getProjects($returnObjects = false, $login = false, $nonExpired = false) {
        ...
        $projects = array();
        foreach ($result as $value) {
            $returnObjects ? $projects[$value['id']] = new EfrontProject($value['id']) : $projects[$value['id']] = $value;
        }
