...
class EfrontSearch
{
	...
	public static function searchForOneWord($word, $maxres, $tableName, $position) {
		$result = eF_getTableDataFlat("search_keywords", "foreign_ID, count(keyword) AS score, table_name, position", "keyword like '%$word%'".($position ? ' and position='.$position : '').($tableName ? ' and table_name='.$tableName : '')."", "", "foreign_ID,table_name limit $maxres");
