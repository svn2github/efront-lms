    ...
    $str .= '<table style = "width:100%;text-align:left">';
                    ...
                    <td>
                        ...
                        <tr>
                            <td class="headerImage" width="2%" align="right">
                                <a href="professor.php?ctg=survey&action=create_survey&survey_action=create&screen=1&lessons_ID='.$_SESSION['s_lessons_ID'].'"><img src="images/16x16/add2.png" title='._CREATESURVEY.' border="0px" /></a>
                            </td>
        ...
        $str .='
            ...
            for($i = 0 ; $i < sizeof($params['data']) ; $i ++){
                    if($params['data'] == '0'){
                        $str.='<tr><td class="emptyCategory" colspan="100%" align="center">'._NOSURVEYFORLESSON.'</td></tr>';
                        break;
                    }else{
                        if(fmod($i,2)){
                            $now = time();
                        ...
                        foreach($params['data'][$i] as $key => $value){
                            ...
                            if($key == 'survey_name'){
                                ...
                                $now = time();
                                if($value == 1 && time() < $params['data'][$i]['end_date']){
                                    $str .= '<td align="center"><a href="professor.php?ctg=survey&action=change_status&survey_action=deactivate_survey&surveys_ID='.$params['data'][$i]['id'].'"><img src="images/16x16/trafficlight_green.png" border="0px" title="'._ACTIVE.'" /></a></td>';
                                }else{
                                    $str .= '<td align="center"><a href="professor.php?ctg=survey&action=change_status&survey_action=activate_survey&surveys_ID='.$params['data'][$i]['id'].'"><img src="images/16x16/trafficlight_red.png" border="0px" title="'._DEACTIVE.'" /></a></td>';
                                }
                        ...
                        if( strcmp($params['user_type'],"professor") == 0 ){
                            $str .='<td align="center"><a href="professor.php?ctg=survey&action=view_users&surveys_ID='.$params['data'][$i]['id'].'">'.$users['done_users'][0]['count(*)'].'/'.$users['total_users'][0]['count(*)'].'</a></td>
                                <td align="center">
                                    ...
                                    <a href="professor.php?ctg=survey&action=preview&surveys_ID='.$params['data'][$i]['id'].'"><img src="images/16x16/view.png" border="0px" title="'._PREVIEW.'"/></a>
                                    <a href="professor.php?ctg=survey&action=statistics&surveys_ID='.$params['data'][$i]['id'].'"><img src="images/16x16/chart.png" border="0px" title="'._SURVEYSTATISTICS.'"/></a>
                                    <a href="'.$params['user_type'].'.php?ctg=survey&surveys_ID='.$params['data'][$i]['id'].'&screen_survey=2"><img src="images/16x16/note_edit.png" border="0px" title="'._EDITQUESTION.'"</a>
                                    <a href="professor.php?ctg=survey&action=delete&surveys_ID='.$params['data'][$i]['id'].'" onclick="return confirm(\''._IRREVERSIBLEACTIONAREYOUSURE.'\');"><img src="images/16x16/delete.png" border="0px" title="'._DELETE.'"/></a>
                                </td>';
                        }
                        if( strcmp($params['user_type'] , "professor") == 0){
