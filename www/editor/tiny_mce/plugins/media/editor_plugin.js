...
//alert(value);		
		...
		switch (command) {
			case "mceMedia":
				tinyMCE.openWindow({
						file : '../../plugins/media/media.php',
						width : 430 + tinyMCE.getLang('lang_media_delta_width', 0),
						height : 670 + tinyMCE.getLang('lang_media_delta_height', 0)
					}, {
						editor_id : editor_id,
						inline : "yes"
				});
...
//alert("insert_to_editor");
				img = tinyMCE.getParam("theme_href") + '/images/spacer.gif';

				content = content.replace(/<script[^>]*>\s*write(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|Audio)\(\{([^\)]*)\}\);\s*<\/script>/gi, '<img class="mceItem$1" title="$2" src="' + img + '" />');				
...
//alert("insert_to_editor_dom");			
				d = inst.getDoc();
				nl = content.getElementsByTagName("img");
				for (i=0; i<nl.length; i++) {
					if (/mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|Audio)/.test(nl[i].className)) {
...
//alert(ci);					
					switch (ci) {
						case 'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000':	// edo metatrepetai se flash... me ti na to diaxoriso?
				...
				for (i=0; i<nl.length; i++) {
					switch (tinyMCE.getAttrib(nl[i], 'type')) {
						case 'application/x-shockwave-flash':
							TinyMCE_MediaPlugin._createImgFromEmbed(nl[i], d, 'mceItemFlash');  //maybe a problem here
							break;
			...
			case "get_from_editor":
				...
				while ((startPos = content.indexOf('<img', startPos+1)) != -1) {
					...
					if (!/mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|Audio)/.test(attribs['class']))
						continue;
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		do {
			if (node.nodeName == "IMG" && /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|Audio)/.test(tinyMCE.getAttrib(node, 'class'))) {
...
/*}else{
	...
	h += '<object classid="clsid:' + cls + '" type="' + mt + '"';
		//h += '<object type="' + mt + '"';
		//h += typeof(p.id) != "undefined" ? ' id="' + p.id + '"' : '';
		h += typeof(p.name) != "undefined" ? ' name="' + p.name + '"' : '';
		h += typeof(p.width) != "undefined" ? ' width="' + p.width + '"' : '';
		h += typeof(p.height) != "undefined" ? ' height="' + p.height + '"' : '';
		h += typeof(p.align) != "undefined" ? ' align="' + p.align + '"' : '';
		h += ' data="/editor/tiny_mce/plugins/media/player.swf" id="audioplayer1">';
	
		h += '<param name="movie" value="/editor/tiny_mce/plugins/media/player.swf"><param name="FlashVars" value="playerID=1&amp;soundFile='+ p.src + '"><param name="quality" value="high"><param name="menu" value="false"><param name="wmode" value="transparent"></object>';
	
}*/
}else{
	
	h += '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0"';
		//h += '<object type="' + mt + '"';
		//h += typeof(p.id) != "undefined" ? ' id="' + p.id + '"' : '';
		h += typeof(p.name) != "undefined" ? ' name="' + p.name + '"' : '';
		h += typeof(p.width) != "undefined" ? ' width="' + p.width + '"' : '';
