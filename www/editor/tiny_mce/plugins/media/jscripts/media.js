...
function init() {
	...
	if (/mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|Audio)/.test(tinyMCE.getAttrib(fe, 'class'))) {
		pl = "x={" + fe.title + "};";
	...
	if (pl != "") {
		...
		switch (type) {
			...
			case "audio":
				setBool(pl, 'audio', 'play');
				setBool(pl, 'audio', 'loop');
				setBool(pl, 'audio', 'menu');
				setBool(pl, 'audio', 'swliveconnect');
				setStr(pl, 'audio', 'quality');
				setStr(pl, 'audio', 'scale');
				setStr(pl, 'audio', 'salign');
				setStr(pl, 'audio', 'wmode');
				setStr(pl, 'audio', 'base');
				setStr(pl, 'audio', 'flashvars');
			break;
...
function insertMedia() {
	...
	fe = tinyMCE.selectedInstance.getFocusElement();
	if (fe != null && /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|Audio)/.test(tinyMCE.getAttrib(fe, 'class'))) {
...
function getType(v) {
	var fo = tinyMCE.getParam("media_types", "flash=swf;audio=mp3,mp4;shockwave=dcr;qt=mov,qt,mpg,mp3,mp4,mpeg;shockwave=dcr;wmp=avi,wmv,wm,asf,asx,wmx,wvx;rmp=rm,ra,ram").split(';'), i, c, el, x;
...
function changedType(t) {
	...
	d.getElementById('audio_options').style.display = 'none';
...
function serializeParameters() {
	...
	switch (f.media_type.options[f.media_type.selectedIndex].value) {
		...
		case "audio":
			s += getBool('audio', 'play', true);
			s += getBool('audio', 'loop', true);
			s += getBool('audio', 'menu', true);
			s += getBool('audio', 'swliveconnect', false);
			s += getStr('audio', 'quality');
			s += getStr('audio', 'scale');
			s += getStr('audio', 'salign');
			s += getStr('audio', 'wmode');
			s += getStr('audio', 'base');
			s += getStr('audio', 'flashvars');
		break;
...
//alert(f.media_type.options[f.media_type.selectedIndex].value);
	switch (f.media_type.options[f.media_type.selectedIndex].value) {
		case "flash":
	...
	if (f.media_type.options[f.media_type.selectedIndex].value == "audio") {
	
		h += '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0" width="' + pl.width + '" height="' + pl.height + '" id="' + pl.id + '" name="' + pl.name + '" align="' + pl.align + '" data="/editor/tiny_mce/plugins/media/player.swf" id="audioplayer1">';
...
/*		for (n in pl) {
		...
		h += '<param name="movie" value="/editor/tiny_mce/plugins/media/player.swf"><param name="FlashVars" value="playerID=1&amp;soundFile="'+ pl.src + '"><param name="quality" value="high"><param name="menu" value="false"><param name="wmode" value="transparent"><embed type="application/x-shockwave-flash" id="audioplayer1" flashvars="playerID=1&amp;soundFile='+ pl.src + '" quality="high" menu="false" wmode="transparent" src="/editor/tiny_mce/plugins/media/player.swf"></embed></object>';
