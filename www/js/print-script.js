...
var colourchange = 0;
// Variable for the last user to merge same user messages
                ...
                if (http_request.status == 200) {
                		...
                		if (http_request.responseText == "ack") {	
                			...
                			//alert("ack");
                	 		...
                	 		var table_style_size = <?php if (preg_match("/compatible; MSIE 6/", $_SERVER['HTTP_USER_AGENT']) || preg_match("/compatible; MSIE 7/", $_SERVER['HTTP_USER_AGENT'])) { ?>"92%"<?php }else{ ?> "100%" <?php } ?> ;
							...
							if (response[0] == "<?php echo _CHATROOMDOESNOTEXIST_ERROR;?>") {
								test.document.getElementById("chat_content").innerHTML = '<table class="chatbox" cellspacing="0" cellpadding="0"  style="width:'+table_style_size+'"><tr><td style="font-size:10px;color:red;" align="left"><?php echo _CHATROOMDELETEDBYOWNER .  "<BR>" . _REDIRECTEDTOEFRONTMAIN; ?></td></tr></table>';
								...
								test.document.getElementById("chat_content").innerHTML = '<table class="chatbox" cellspacing="0" cellpadding="0"  style="width:'+table_style_size+'"><tr><td style="font-size:10px;color:red;" align="left"><?php echo _CHATROOMHASBEENDEACTIVATED . "<BR>" ._REDIRECTEDTOEFRONTMAIN;?></td></tr></table>';
								...
								for (i = response.length-5; i >= 0; i = i - 4) {
									if (response[i] != '') {

										if ($('last_spoken_login').value == response[i]) {
											test.document.getElementById("chat_content").innerHTML = test.document.getElementById("chat_content").innerHTML + '\n<table class="chatbox" cellspacing="0" cellpadding="0" bgcolor="'+ color+'" style="width:'+table_style_size+'"><tr><td style="color:black;font-size:10px;">'+ response[i+3] + '</td></tr></table>';
										} else {
											if (colourchange) {
												colourchange = 0;
												color = "#F9F9F9";
											...
											test.document.getElementById("chat_content").innerHTML = test.document.getElementById("chat_content").innerHTML + '\n<table class="chatbox" cellspacing="0" cellpadding="0"  bgcolor="'+ color+'" style="width:'+table_style_size+'"><tr><td style="' + response[i+2] + 'font-size:10px;" align="left">' + response[i] + '</td><td style="'+ response[i+2] + ';font-size:10px;"" align="right">'+ response[i+1] + '</td></tr><tr><td colspan="2" style="color:black;font-size:10px;">'+ response[i+3] + '</td></tr></table>';
											$('last_spoken_login').value = response[i];
										}	
				                        test.document.getElementById('chat_content').scrollTop=test.document.getElementById('chat_content').scrollHeight;
			                        }
		                        }
		                    } 
		                }       
...
function show_user_box(user_str,user,send_msg,view_page,user_type,user_time)
{
    //if (user != cur_user && user != '') {
        href_str = '<a href = "forum/new_message.php?recipient='+user+'" onclick = "eF_js_showDivPopup(\'\', \'\', \'user_table\');eF_js_showDivPopup(\'\', new Array(\'750px\', \'450px\'))" target = "POPUP_FRAME">'+send_msg+"</a>";
...
function initSidebar(s_login)
{
    ...
    else
    {
        ...
        if(is_ie == "true")
        {
            ...
            top.sideframe.document.getElementById('toggleIconImageDown').style.position="absolute";
            top.sideframe.document.getElementById('toggleIconImageDown').style.left = "1px";
            top.sideframe.document.getElementById('toggleIconImageDown').style.top = "4px";            
        }
        top.sideframe.document.getElementById('logoutImage').style.position="absolute";
...
function toggleSidebar(s_login)
{
	...
	var i = 2;
        ...
        // Visible icon is a global variable used to buffer which icon was enabled before the toggling
        if (visibleIcon == "true") 
	    $('toggleIconImageDown').style.visibility = "visible";
	...
	if ($('toggleIconImageDown').style.visibility == "visible") {
            visibleIcon = "true";
        }    
	$('toggleIconImageDown').style.visibility = "hidden";        
    }
}
*/
