...
if($_SESSION['s_type'] == "administrator"){
	...
	$command_array = array("edit unit" => G_SERVERNAME."professor.php?ctg=content&edit_unit=",
						...
						"preview test" => G_SERVERNAME."professor.php?ctg=tests&show_test=",
...
$command_array["send message"]= G_SERVERNAME."forum/new_message.php";
...
if (isset($_POST['search_text'])) {
	...
	//pr($query_array[0]);pr($command_array_keys[$right_key]);return;
		if (strcmp($query_array[0], $command_array_keys[$right_key]) == 0){
			header("location:".$command_array_values[$right_key]);return;
		}else{
			...
			if(strpos($opcode,"lesson")!== false){
				...
				if(sizeof($result_command) == 1){
				 	header("location:".$command_array_values[$right_key].$result_command[0]['id']);return;
				...
				if(sizeof($result_command) == 1){
				 	header("location:".$command_array_values[$right_key].$result_command[0]['login']);return;
				}else if(sizeof($result_command) > 1){
					$smarty -> assign("T_SEARCH_COMMAND", $result_command);
					$smarty -> assign("T_SEARCH_COMMAND_LOCATION", $command_array_values[$right_key]);
				...
				$result_command = eF_getTableData("tests,content","tests.id,content.name,content.lessons_ID","tests.content_ID=content.id and content.name like'%".$argument."%'");
				 if(sizeof($result_command) == 1){
				 	header("location:".$command_array_values[$right_key].$result_command[0]['id']."&lessons_ID=".$result_command[0]['lessons_ID']);return;
				 }else if(sizeof($result_command) > 1){
					$smarty -> assign("T_SEARCH_COMMAND", $result_command);
					$smarty -> assign("T_SEARCH_COMMAND_LOCATION", $command_array_values[$right_key]);
					$smarty -> assign("T_SEARCH_COMMAND_KEY1", "id");
				...
				$result_command = eF_getTableData("questions","id,text,type,lessons_ID","text like'%".$argument."%'");
				 if(sizeof($result_command) == 1){
				 	header("location:".$command_array_values[$right_key].$result_command[0]['id']."&question_type=".$result[0]['type']."&lessons_ID=".$result_command[0]['lessons_ID']);return;
				}else if(sizeof($result_command) > 1){
					$smarty -> assign("T_SEARCH_COMMAND", $result_command);
					$smarty -> assign("T_SEARCH_COMMAND_LOCATION", $command_array_values[$right_key]);
					$smarty -> assign("T_SEARCH_COMMAND_KEY1", "id");
    ...
    if (!$search_results_data AND !$search_results_forum AND !$search_results_pmsgs AND !$search_results_courses AND !$result_command) {
